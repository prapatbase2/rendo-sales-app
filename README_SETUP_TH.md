# Rendo PWA v1.2.0 — คู่มือติดตั้งและอัปเดตสำหรับผู้เริ่มต้น
...
- แก้ต้นเหตุหน้า **ลงวันทำงานค้าง**: เดิมการโหลดแบบฟอร์มและรายการใช้ตัวนับคำสั่งร่วมกัน เมื่อเปลี่ยนวันที่หรือพนักงานเร็ว ๆ งานหนึ่งจะยกเลิกอีกงาน แต่ข้อความ “กำลังโหลด” ไม่ถูกปิด
- แยกงานโหลดแบบฟอร์มวันทำงานกับรายการรายเดือนออกจากกัน และยกเลิกเฉพาะผลลัพธ์เก่าที่ไม่ตรงกับหน้าปัจจุบัน
- หน้าเปิดทันที ไม่รอ Firestore ทั้งชุด; หากเครือข่ายช้าจะแสดงข้อความและปุ่มลองใหม่แทนการหมุนค้าง
- ลดจุดล้มเหลวของหน้าลงทำงาน โดยค้นข้อมูลตามเดือนแล้วกรองพนักงานในเครื่อง จึงไม่ต้องสร้าง Composite Index ใหม่สำหรับ attendance
- ป้องกันข้อมูลจากหน้าที่เปิดก่อนหน้ามาเขียนทับหน้าใหม่เมื่อกดเปลี่ยนเมนูเร็ว ๆ
- เพิ่ม cache อายุสั้นและ timeout สำหรับการอ่านข้อมูล พร้อมล้าง cache หลังบันทึก เพื่อให้หน้าเดิมเปิดซ้ำเร็วขึ้นแต่ยังเห็นข้อมูลล่าสุด
- ช่องตัวเลขที่เป็น `0` จะล้างให้อัตโนมัติเมื่อแตะ/คลิก จึงพิมพ์จำนวนใหม่ได้ทันที; ถ้าปล่อยว่าง ระบบจะคืนเป็น `0`
- เงินสดเปิดกะดึงจาก **ยอดปิดกะล่าสุดของวันที่ร้านเปิดก่อนหน้า** แม้เมื่อวานหยุด โดยค้นย้อนหลังได้หลายเดือน และยังแก้ไขเองได้
- ปรับการบันทึกวันทำงาน ยอดขาย เบิกเงิน และรายจ่ายให้คืนสถานะปุ่มเสมอ พร้อมไม่รอ Audit/Auto Backup จนหน้าหลักค้าง
- รักษาการแก้จาก v1.1.0: ค่าตอบแทนทีละคน, เพดานประกันสังคม, PDF ครบถึงยอดโอน และแชร์ผ่านเมนูแชร์ของมือถือ

## วิธีอัปเดตจาก v1.0–v1.1.0 โดยไม่ลบข้อมูล
...
2. แตกไฟล์ ZIP v1.2.0
3. อัปโหลดไฟล์ทั้งหมดในโฟลเดอร์ `rendo_pwa_v1_2_0` ไปแทนไฟล์เดิมบน GitHub Pages หรือ Firebase Hosting รวมถึง `app.js`, `style.css`, `index.html`, `sw.js`, `manifest.webmanifest` และโฟลเดอร์ `icons`
4. ถ้าใช้ Firebase Hosting ให้รัน `firebase deploy --only hosting`
5. รอระบบเผยแพร่ แล้วเปิด URL แอปโดยเติม `?v=1.2.0` ต่อท้ายหนึ่งครั้ง เช่น `https://ชื่อผู้ใช้.github.io/rendo-pwa/?v=1.2.0`
6. ตรวจมุมบนของแอปว่าขึ้น **v1.2.0**
...
7. ลากไฟล์และโฟลเดอร์ทั้งหมดใน `rendo_pwa_v1_2_0` ลงไป รวมถึงโฟลเดอร์ `icons`
8. กด **Commit changes**
