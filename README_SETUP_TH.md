# Rendo PWA v1.1.1 — คู่มือติดตั้งและอัปเดตสำหรับผู้เริ่มต้น
> อัปเดต v1.1.1: แก้หน้าลงทำงานค้างจากการคำนวณวันที่ในเขตเวลาไทย ปรับโหลดแบบไม่ล็อกหน้า และปรับช่องยอดขายเลข 0 ให้พิมพ์ทับได้ทันที
...
- แก้สาเหตุหลักของหน้าลงวันทำงานค้างในเขตเวลาไทย: การเพิ่มวันแบบเดิมแปลงกลับผ่าน UTC ทำให้วันที่ไม่เดินหน้าและวนลูปไม่จบ
- เปลี่ยนการตรวจวันที่ยังไม่ได้ลงให้วนตามเลขวันที่แบบมีขอบเขต 1–31 จึงไม่สามารถค้างจากลูปวันที่ได้
- แยกสถานะโหลดฟอร์มกับรายการลงวันทำงาน ไม่ล็อกทั้งฟอร์ม และป้องกันคำสั่งเก่าทับหน้าปัจจุบัน
- ช่องตัวเลขหน้ายอดขายที่เป็น 0 สามารถแตะแล้วพิมพ์เลขใหม่ทับได้ทันที หากปล่อยว่างจะกลับเป็น 0
- เงินเปิดกะค้นจากยอดปิดกะของวันเปิดร้านก่อนหน้า โดยข้ามวันที่หยุด และยังแก้ไขเองได้
- เพิ่ม cache ระยะสั้นและการยกเลิกผลโหลดเก่า ทำให้สลับหน้าเร็วขึ้นและลด error จากหน้าที่ถูกปิดไปแล้ว
...
2. แตกไฟล์ ZIP v1.1.1
3. อัปโหลดไฟล์ทั้งหมดในโฟลเดอร์ `rendo_pwa_v1_1_1` ไปแทนไฟล์เดิมบน GitHub Pages หรือ Firebase Hosting รวมถึง `app.js`, `style.css`, `index.html`, `sw.js`, `manifest.webmanifest` และโฟลเดอร์ `icons`
4. ถ้าใช้ Firebase Hosting ให้รัน `firebase deploy --only hosting`
5. รอระบบเผยแพร่ แล้วเปิด URL แอปโดยเติม `?v=1.1.1` ต่อท้ายหนึ่งครั้ง เช่น `https://ชื่อผู้ใช้.github.io/rendo-pwa/?v=1.1.1`
6. ตรวจมุมบนของแอปว่าขึ้น **v1.1.1**
...
7. ลากไฟล์และโฟลเดอร์ทั้งหมดใน `rendo_pwa_v1_1_1` ลงไป รวมถึงโฟลเดอร์ `icons`
8. กด **Commit changes**
